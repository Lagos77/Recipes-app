...
struct CreateRecipeView: View {
    ...
    private func createRecipe() {
        guard let uid = firebase.auth.currentUser?.uid else {return}
        
        let ref = firebase.storage.reference(withPath: uid)
    ...
    private func recipeData(recipeImageURL: URL){
        ...
        firebase.firestore.collection("users").document(uid).collection("recipes").document(titleRecipe).setData(recipeData) { err in
            if let err = err {
                print(err)
                return
            }
